    ...
    <nav class="p-5 space-y-1">
        ...
        @if(Auth::guard('petugas')->check())
            ...
            @if(in_array($userLevel, ['Owner','Administrasi']))
                ...
                <a href="{{ route('bahan.index') }}" class="sidebar-link flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all {{ request()->routeIs('bahan.*') ? 'bg-gradient-to-r from-blue-50 to-indigo-50 text-blue-700 shadow-sm border-l-4 border-blue-500' : 'text-gray-700 hover:bg-gray-100' }}">
                 ...
                 <a href="{{ route('produk.index') }}" class="sidebar-link flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all {{ request()->routeIs('bahan.*') ? 'bg-gradient-to-r from-blue-50 to-indigo-50 text-blue-700 shadow-sm border-l-4 border-blue-500' : 'text-gray-700 hover:bg-gray-100' }}">
                    <i class="fas fa-cubes w-5"></i> Produk
                </a>
